...
var aggregationPipeline = require('./../../libs/aggregationPipeline');


//shortId to generate unique ticketNumber
var shortid = require('shortid');

//nodemailer for sending mails on new answer to question
var nodemailer = require('nodemailer');

//creating new instance of event emitter
var events = require('events');
var eventEmitter = new events.EventEmitter();

//importing jwt auth middleware
var auth = require('./../../middleware/jwtAuth').auth;
...
eventEmitter.on('sendMail' , function(data){
    ...
    var mailOptions = {
        from: 'QueryHub <QueryHub.com>',
        to: data.email,
        subject: 'QueryHub Notification',
        text: 'Answer received for ticket number : '+ data.ticketNumber,
        html: '<h1>Hello '+data.name+'</h1><br><h2>You just receievd an answer received for query , ticket number : '+data.ticketNumber+'</h2>'
    } 
...
queryRouter.get('/all' , auth, function(req , res){
    ...
    User.aggregate(aggregationPipeline.all , function(error,result){
        if(err){
            ...
            var err = responseGenerator.generate(true , "No result found , empty array", 204, null);
            res.send(err);
        }else{
            var response = responseGenerator.generate(false, "All queries fetched successfully", 200, result);
            res.send(response);
        }
    });
...
queryRouter.post('/Ticket/:tno/statusChange' , auth , function(req , res){
    ...
    User.findOne({"queries.ticketNumber":req.params.tno} , function(error,result){
        if(err){
            var err = responseGenerator.generate(true , "Some error occured : "+error, 500, null);
            res.send(err);
        }else if(result === null || result === undefined || result === []){
            ...
            var err = responseGenerator.generate(true , "No queries raised yet", 204, null);
            res.send(err);
        }else{
            ...
            if(status === "Open"){
                ...
                var newStatus = "Close";
                eventEmitter.emit('sendMailForStatusChange' , {status : "Closed" , tno : tno , email : email, name : name});
            }else{
                result.queries[index].ticketStatus = "Open";
                newStatus = "Open";
                eventEmitter.emit('sendMailForStatusChange' , {status : "Reopened" , tno : tno, email : email, name : name});
            }
            result.save(function(error){
                if(error){
                   ...
                   var response = responseGenerator.generate(false, "Ticket status chnaged successfully to : "+newStatus, 200, result);
...
.get( auth ,function(req , res){
    ...
    User.aggregate(aggregationPipeline.currentQuery(req.params.tno) , function(error,result){
        if(err){
            ...
            var response = responseGenerator.generate(false, "Query fetched successfully", 200, result);
            // console.log(response)
            res.send(response);
        }
    });
        ...
        if(err){
            ...
            result.save(function(error){
                if(error){
                   ...
                   var response = responseGenerator.generate(false, "New message thread created successfully", 200, result.queries[index]);
                   res.send(response); 
                }
            });
        ...
        if(err){
            ...
            result.save(function(error){
                if(error){
                   ...
                   var response = responseGenerator.generate(false, "Query edited successfully", 200, result);
                   console.log(response)
                   res.send(response); 
                }
            });
        ...
        if(err){
            ...
            result.save(function(error){
                if(error){
                   ...
                   var response = responseGenerator.generate(false, "Query deleted successfully", 200, result);
                   console.log(response)
                   res.send(response); 
                }
            });
...
.get( auth ,function(req , res){
    User.find({} , function(error , allUsers){
        if(error){
            ...
            var err = responseGenerator.generate(true , "No users yet", 500, null);
            res.send(err);
        }else{
            var response = responseGenerator.generate(false , "All users' details fetched successfully", 200, allUsers);
            res.send(response);
        }
    });
});//end get all users
...
.get( auth ,function(req , res){
    ...
    User.aggregate(aggregationPipeline.currentUser(userId) , function(error,result){
        if(err){
            ...
            var err = responseGenerator.generate(true , "No queries asked", 204 , null);
            res.send(err);
        }else{
            var response = responseGenerator.generate(false, "All queries of "+result[0].name+" fetched successfully", 200, result);
            res.send(response);
        }
    });
            ...
            user.save(function(error){
                if(error){
                   ...
                   res.send(err); 
                    ...
                    console.log(user)
                   var response = responseGenerator.generate(false, "User fetched", 200, data.ticketNumber);
                   res.send(response);
                }
            });
...
var pushFile = function(tno , file , res){
    ...
    User.findOne({"queries.ticketNumber":tno} , function(error,result){
        console.log("before2")
        if(err){
            var err = responseGenerator.generate(true , "Some error occured : "+error, 500, null);
            console.log("before3")
            ...
            result.save(function(error){
                if(error){
                   ...
                   var response = responseGenerator.generate(false, "File pushed successfully", 200, result);
                }
            });
